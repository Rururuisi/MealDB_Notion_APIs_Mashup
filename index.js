...
function get_food_recipes(keyword, state, redirect_uri, res) {
    ...
    https.request(endpoint, { method: "GET" }, (recipes_stream) => {
        process_stream(recipes_stream, receive_food_recipes, keyword, state, redirect_uri, res)
    }).end();
}

function receive_food_recipes(body, keyword, state, redirect_uri, res) {
    const {meals} = JSON.parse(body);
    if (meals == null) {   // no search result
...
function request_access_token(authCode, res, redirectUri, session) {
    ...
    let auth_base64 = Buffer.from(`${client_id}:${client_secret}`).toString("base64");
    const options = {
        method: "POST",
        headers: {
...
function receive_access_token(body, session, res) {
    ...
    if (access_token === undefined) {     // when received error info
    ...
    create_keyword_page(access_token, session.food_recipes, session.keyword, res);
}
...
function create_keyword_page(token, food_recipes, keyword, res) {
    ...
    let recipe_count = 0;
    create_notion_page(token, post_data, add_recipes, food_recipes, recipe_count, res);
}
...
function process_recipe(parent_page, token, food_recipes, count, res) {
    ...
    recipe_page.children[3].paragraph.rich_text[0] = { "text": { "content": recipe.ingredients } };
    ...
    create_notion_page(token, post_data, wait_for_page_response, parent_page, food_recipes, count, res);
}


function wait_for_page_response(body, token, parent_page, food_recipes, count, res) {
    count++;
...
function create_notion_page(token, post_data, callback, ...args) {
    ...
    https.request(page_endpoint, options, (page_stream) => {
