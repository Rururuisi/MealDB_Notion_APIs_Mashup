...
function request_handler(req, res) {
	...
	// route for collect user input
		...
		const { keyword, page_url } = url.parse(req.url, true).query;
		if (keyword.trim() == "") {
			error_status(res, 400, "Bad Request", "Empty Search");
			return;
		}
		const index = page_url.lastIndexOf("-");
		const page_id = page_url.slice(index + 1);
		const state = crypto.randomBytes(20).toString("hex");
		all_sessions.push({ keyword, page_id, state });
...
function create_keyword_page(token, page_id, food_recipes, keyword, res) {
	...
	keyword_page.parent.page_id = `${page_id}`;
